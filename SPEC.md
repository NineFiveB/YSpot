...
2. **Merge never moves the selection.** Cross-source merge inserts rows by global score — the shell's ranker, which applies frecency from the per-user store uniformly across all sources (§5 preamble) — but MUST NOT move or re-index the currently selected row. Selection is sticky to its stable ID (§5.6) and resets to row 0 **only on generation change** (a new keystroke), never on a `seq` append or a late-arriving source. **Built-in commands (§7.6) score one band above the shared match scale.** A built-in whose own *name* matches at `WORD_START` or better takes a fixed `+0.5`, placing it in `[1.30, 1.50]` — above every file, app and settings row (`EXACT 1.0` plus the `0.09` frecency ceiling) and below the calculator's `2.0`; a hit that reaches a built-in only through a synonym stays on the shared scale. (**Amended 2026-09-07**, superseding the reading of "inserts rows by global score" under which every source shares one scale. A built-in is a hand-written destination the user typed the name of, and it was losing to whatever the filesystem happened to contain: 101 entries under one user profile are named exactly `Settings`, and an exact match on a directory at depth 6 scores 0.89 against a command's flat 0.800, so `YSpot Settings` sat at row 7 of an 8-row viewport. The cost is that the built-ins now win their own names outright — `file` puts File Search above every file, `clipboard` puts Clipboard History above a folder called Clipboard. That is the intent, and it is also the whole risk; the guard against it spreading further is that only a name match is promoted, which is why `documents` still returns the user's Documents folder first.) Two built-ins that tie exactly are ordered as the list declares them, not by name: `windows` reaches Windows Settings and Windows Backup at the same prefix score plus the band, and `settings` reaches YSpot Settings and Windows Settings the same way. (**Amended 2026-09-09**, adding a sixth built-in, `windows.backup`, which opens the Windows Backup app by AUMID; its `AppsFolder` row is suppressed as the command's duplicate, exactly as the Settings app's is. The ask was that Windows Backup sit directly under Windows Settings for `windows`, and on the shared scale it could not: it prefix-matched at 0.9 beside eleven other "Windows …" apps and four catalog pages, and the name order put it at row 4 — twice, since the `ms-settings:sync` page is also called Windows backup. Two general fixes were tried and rejected on review. Ranking settings pages above apps on a tie put Taskbar above Task Manager, Notifications above Notepad and Camera privacy above the Camera app. Demoting the `C:\Windows` directory row only promoted the next directory named Windows, of which any profile has several. The built-in list is the mechanism this contract already has for "a hand-written destination the user typed the name of", and it is what put Windows Settings there. Three things distinguish an app's built-in from YSpot's own. It is offered only while its AUMID is in `AppsFolder`, decided per query because the app catalog is empty at startup and fills on its own thread — the app is Windows 11 22H2+, this spec supports Windows 10, and a top row whose Enter fails is worse than no row. It takes the band from `PREFIX` only, not `WORD_START`: `ba` word-starts "Windows Backup" and would otherwise open it from two letters, above Background and Battle.net; `backup` still finds it, on the shared scale. And its `AppsFolder` row is suppressed by AUMID, exactly as the Settings app's is. The cost, stated in full: every prefix of "Windows" from `wi` up now shows two band rows instead of one, so a user typing `win` toward WinRAR or Windows Terminal is one row further from it — the same cost Windows Settings already imposed on those prefixes, one entry deeper. This is a decision in code, not data, and a third such entry should be argued for as hard as this one was. The declaration-order tie-break is the only kind-aware ordering in the merge, is confined to built-ins, and is the fresh-profile default: frecency is added to built-ins as to every row, so once either has been launched from YSpot the one used more leads, which is §7.1 working within a tier as it does everywhere.)
...
4. **The root list carries a per-kind budget.** After the global-score sort, the root list keeps at most `calc 1, command 2, setting 3, app 3, window 2, file 3` rows of each kind; a row that is frozen under rule 3 or currently selected is never trimmed, and counts against its kind's budget rather than being exempted from it. The surviving rows are a *subsequence* of the global-score order, so rules 1-3 hold verbatim — a budget removes rows, it never moves one. (**Amended 2026-09-07**, superseding the reading of rule 2's "Cross-source merge inserts rows by global score" as a complete description of the display list. Every source contributed its quota unconditionally — 1 calc + 8 apps + 4 settings + 3 commands + 5 windows + 5 files = 26 rows into a viewport of 8.67 — so a query matching five kinds pushed the row the user typed for off the visible page. Per-source caps (§7.1, §7.2, §7.3, §4.7) are unchanged and are now candidate pools. The cost, stated plainly: a fourth matching app or settings page is not in the root list and nothing on screen says so; §7.3's File Search view is the escape hatch for files, and there is none for apps or settings. Per-kind section headers were considered and deferred, not rejected: they make display order primarily *kind*, which contradicts rule 2's plain text, and they restructure the listbox §5.12's `aria-activedescendant` pattern depends on at a moment when screen-reader verification is itself deferred, so a regression there would have nobody to catch it.)
...
- **Settings pages.** YSpot ships a static catalog of `ms-settings:` URIs (name, synonyms, keywords, URI) derived from Microsoft's published reference ("Launch the Windows Settings app", learn.microsoft.com). Launch is `ShellExecuteEx` on the URI (e.g. `ms-settings:display`, `ms-settings:privacy-microphone`). The catalog format MUST be data-driven (JSON shipped with the app) so it can be updated without a code release. Catalog display names and synonyms are English-only in v1 — an explicit decision; localization of the catalog is deferred. The Settings **home page** (`ms-settings:`) is a built-in command (§7.6), not a catalog entry. (**Amended 2026-09-07**, superseding the catalog's `ms-settings:` entry, now deleted from `resources/settings-catalog.json`. The home page is not one of ninety-odd deep links, it is *the* destination people mean when they type `settings`, and it needs §5.11's command band to reach the top of that query. The cost is real: that entry's display name and synonyms are now code and can no longer be corrected without a release. It is one entry of 94, it is the one whose name will never need correcting, and the other 93 stay data. The `control` synonym is dropped rather than carried over — it aimed the word at the Settings app, and the catalog's twenty Control Panel items are the right answer for it.)
...
- **Root query.** Top-level queries include inline fuzzy filename matches from yspot-indexd (substring/fuzzy/camel-case match quality), interleaved with apps/settings/commands under the shell's global ranker, which applies per-user frecency uniformly across all sources (§7.1's store). Inline file results are capped to keep the root list scannable: at most 5 candidates survive per generation, same-named rows collapse to the best-scoring one, and at most 3 are displayed (§5.11 r4). (**Amended 2026-09-07**, superseding "Inline file results are capped (default 5) to keep the root list scannable." The number was already a default rather than a contract, so it is not the change; the collapse is. 101 directories under one user profile are named exactly `Settings` and five were shown as five rows. The collapse is scoped to the root list only — in the File Search view, five `main.rs` in five projects are five different answers, and that stays true.)
