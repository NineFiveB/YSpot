...
- **Candidate generation (prefilter):** each expensive tier scores only a prefiltered candidate set, never the full entry table. Substring candidates come from a SIMD `memmem` linear scan over the case-folded arena (a 1M-name arena scans in single-digit ms). Fuzzy candidates come from **trigram posting lists** built over the folded arena: candidates = names containing **all** query trigrams; for queries shorter than 3 characters the fuzzy tier does not run — only the substring and higher tiers apply. Camel-case candidates come from a precomputed **word-boundary-initials arena** (the first letter of each name segment, stored in a parallel arena and scanned by prefix/substring). Fuzzy DP scoring runs only on the union of these candidate sets. These structures are budgeted inside the 200 MB / 1M-entry cap above. §10 M0 exit criteria include worst-case 2-char and 3-char fuzzy query measurements, not just substring.
...
- **Storage.** Encrypted-at-rest SQLite (SQLCipher); the database key is generated per-user and protected with DPAPI (`CryptProtectData`, `CRYPTPROTECT_UI_FORBIDDEN`). Retention is configurable (default 30 days / 10,000 entries; images capped at 500 MB total, oldest-first eviction).
- **Paste.** Selecting an entry hides the popup, restores focus to the previous foreground window, writes the entry to the clipboard, and injects Ctrl+V via `SendInput`. "Paste as plain text" writes `CF_UNICODETEXT` only. This history is deliberately independent of Windows' Win+V cloud clipboard: no `Windows.ApplicationModel.DataTransfer` history APIs, no cloud sync, and it works with Win+V disabled.
...
- **Machine B**: ~2015 dual-core (i5-5200U class), 8 GB RAM, SATA SSD, 60 Hz display, Windows 10 1809.
The CI latency harness runs on Machine B.

**Exit criteria (measured, not eyeballed):** hotkey→visible < 50 ms p95; **results data available to the frontend ≤ 20 ms p95 after keydown, applied in the next rAF after arrival** (per §2.5's decomposition; end-to-end pixels ≈ budget + up to 2 vsync periods — ~33 ms at 60 Hz, ~16 ms at 120 Hz) on a ≥1M-file volume — measured for substring queries **and** worst-case 2-char and 3-char fuzzy queries; initial index of 1M files < 15 s on the SSD reference machine with quiescent disk (relaxed HDD target: 90 s); USN event→searchable < 1 s; service RSS < 200 MB at 1M files (filename-index budget, §1.2). Numbers recorded on both reference machines. **If any budget fails, M1 does not start** — architecture is revisited instead.
