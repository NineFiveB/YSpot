...
- **Candidate generation (prefilter):** each expensive tier scores only a prefiltered candidate set, never the full entry table.
  ...
  - **Fuzzy subsequence** candidates come from a **bit-sliced character-class index**: 64 slices, one bit per slot per class, and a candidate must have a class set that is a **superset of the query's**. Queries shorter than 3 characters do not run the fuzzy tier at all.
    ...
    **Bounded by the candidate cap.** Because the prefilter is deliberately wider, a query whose character classes are common can produce more survivors than the fuzzy cap will verify, in which case the retained set is the shallowest — the cap, not the prefilter, then decides what is missed. This is a *missing result*, not a slow one, so no latency budget can catch it: the benchmark harness therefore reports survivors against the cap per query class, and on the synthetic 1M corpus the widest class admits 572k candidates against a cap of 20k. Tracked as issue #7.
