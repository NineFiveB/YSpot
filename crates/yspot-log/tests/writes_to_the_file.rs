...
fn a_logged_line_lands_in_the_file_at_the_given_path() {
    ...
    assert_eq!(lines.len(), 2, "expected two records, got:\n{text}");

    let first: serde_json::Value = serde_json::from_str(lines[0]).expect("line 1 is not JSON");
    ...
    let second: serde_json::Value = serde_json::from_str(lines[1]).expect("line 2 is not JSON");
